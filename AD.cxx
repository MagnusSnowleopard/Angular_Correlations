...
using namespace std;
#ifndef __CINT__ 
// Reads an integer option safely with validation
int ReadMenuOption()
{
	int optnum = -1;

	while (true) {
		std::printf("Please enter Input Option : ");

		int rc = std::scanf("%d", &optnum);
		if (rc != 1) {
			std::printf("Invalid input (must be an integer). Try again.\n");

			// Clear invalid token(s) from stdin
			int c;
		...
		if (optnum < 0) {
			std::printf("Negative numbers are not allowed! Re-enter Option.\n");
			continue;
		}

		if (optnum > 3) {
			std::printf("Invalid option. Valid options are 0, 1, 2, 3.\n");
			continue;
		}
...
// Returns false when user chooses to quit (option 3)
bool ExecuteMenuOption(
		int optnum,
		int param,
		const std::vector<double>& tdelta,
		const std::vector<double>& chisqr,
		const std::vector<double>& dangler,
		const std::vector<double>& dydatas,
		const std::vector<double>& deydata,
		const double* residual,
		int residual_len
		)
{
	if (param != 1) {
		std::cout << "Invalid parameter input, something is wrong\n";
		return true; // keep running menu
	}
	...
	switch (optnum) {
		...
		case 1: {
				// Chi-squared GUI (your existing class)
				HistoGUI gui;

				// x = atan(delta), y = log(chi^2)
		...
		case 2: {
				// Angular distribution GUI (improved GUI_AD.h class)
				HistoGUIad gui_ad;

				// NOTE: If dangler is degrees, use label "Theta (deg)" instead.
				gui_ad.SetLabels("Theta (rad)", "W(theta) / A0", "Angular Distribution + Fit");

				gui_ad.SetData(dangler, dydatas);
				gui_ad.SetErrors(deydata);

				if (residual != nullptr && residual_len >=3) {
					gui_ad.SetFit(residual[0], residual[1], residual[2]);
				} else {
					std::cout << "Warning: residual vector has fewer than 3 values; fit overlay not set.\n";
				}
...
int main(int argc,char **argv){
	...
	*/
	...
	//
	//  // Initialize / show menu once (same behavior)
	menu();

	// Keep same options, but cleaner handling
	while (true) {
		int optnum = ReadMenuOption();

		bool keep_running = ExecuteMenuOption(
				optnum,
				param,
				tdelta, chisqr,
				dangler, dydatas, deydata,
				residual, n
				);
		...
		// If you want to reprint menu after each action, uncomment:
		menu();
	}
